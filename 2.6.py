...
with open("sentiment-train.csv", "r") as f:
	...
	for row in reader:
		...
		trainTweet.append(row[1].lower().rstrip())

stop_words = stopwords.words('english')
for i in range(len(trainTweet)):
	trainTweet[i] = [w for w in word_tokenize(trainTweet[i])]


wvModel = Word2Vec(sentences=trainTweet, size=300)
...
for tweet in range(len(trainTweet)):
	...
	for word in trainTweet[tweet]:
		if word in wvModel.wv.vocab:
			count += 1
			vec = vec + wvModel[word]
	if count != 0:
		average.append(vec/count)
		proTrainsentiment.append(trainSentiment[tweet])
# pprint(average, indent=4)
clf = LogisticRegression(max_iter=1000)
clf.fit(average, proTrainsentiment)
...
for tweet in range((len(testTweet))):
	...
	for word in testTweet[tweet]:
		if word in wvModel.wv.vocab:
			...
			vec = vec + wvModel[word]
	if count != 0:
		testTweetVec.append(vec/count)
		proTestsentiment.append(testSentiment[tweet])
testPredict = []
...
for i in range(len(testPredict)):
	if str(testPredict[i][0]) == str(proTestsentiment[i]):
		count += 1
